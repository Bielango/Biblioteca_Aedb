    ...
    public class GerirUsuario
    {
        ...
        public static GerirUsuario? Seleciona(int Codigo)
        {
            using (var oCn = DataHelper.Conexao())
            {
                ...
                string SQL = $"select id, Nome from Usuario where id={Codigo}";
                SqlCommand comando = new SqlCommand(SQL, oCn);
                SqlDataReader oDr = comando.ExecuteReader();
                while (oDr.Read())
                {
        ...
        public static void IncluirGerirUsuarioStatico(GerirUsuario oGerirUsuario)
        {
            ...
            //{
            //    throw new Exception($"O código informado está sendo utilizado no gênero {oGerirUsuarioSelecionado.Nome}.");
            //}
            //else
            //{ 
        ...
        public static void Alterar(GerirUsuario oGerirUsuario)
        {
            using (var oCn = DataHelper.Conexao())
            {
                string SQL = $"update Usuario set Nome='{oGerirUsuario.Nome.Replace("'", "")}' where id={oGerirUsuario.id}";
                SqlCommand comando = new SqlCommand(SQL, oCn);
                comando.ExecuteNonQuery();
            }
